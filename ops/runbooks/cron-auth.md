...
```
mkdir -p secrets
printf 'export CLAUDE_CODE_OAUTH_TOKEN=%s\n' 'sk_ここに貼る' > secrets/oauth-token.env
chmod 600 secrets/oauth-token.env
```
※ `secrets/` は .gitignore 済み(git check-ignore で確認済み)なのでGitには絶対入らない。
...
## トークン期限切れ時(約1年後)
`claude setup-token` を再実行し、手順2で `secrets/oauth-token.env` を上書きする。

## setup-token の打ち方の注意
- **1行で** `claude setup-token` と打つ(改行で割れると別コマンド扱いになり失敗する)
- 普通のターミナルで実行する(ブラウザ認証＋コード入力の対話があるため、`!`実行や非対話環境では完結しないことがある)
